...
function ask_rstr() {
read -p "Please enter 1 to clone and build rstrd or enter 2 to download pre-built executables. (1/2)" askrstr
if [ $askrstr = '1' ] || [ $askrstr = 'one' ] || [ $askrstr = 'One' ]; then
	install_rstr
fi
if [ $askrstr = '2' ] || [ $askrstr = 'two' ] || [ $askrstr = 'Two' ] || [ $askrstr = '' ]; then
	download_rstr
fi
}
...
function download_rstr() {
echo "Downloading pre-built RSTR Executables..."
wget https://cdn.discordapp.com/attachments/427617097405693953/532742743496065024/rstr.tar.gz
echo "Extracting..."
tar -zxvf rstr.tar.gz
echo "Deleting rstr.tar.gz..."
rm -rf rstr.tar.gz
echo "Moving executables to proper location..."
sudo mv rstr* /usr/local/bin
}
